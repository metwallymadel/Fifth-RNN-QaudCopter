...
class Task():
    ...
    def get_reward(self):
        ...
        reward -= abs(current_position[0]-self.target_pos[0])
        reward -= abs(current_position[1]-self.target_pos[1])
        reward -= 10*abs(current_position[2]-self.target_pos[2])**2

        # link velocity to residual distance
        #reward -= abs(abs(current_position-self.target_pos).sum() - abs(self.sim.v).sum())

        distance = np.sqrt((current_position[2]-self.target_pos[2])**2)
